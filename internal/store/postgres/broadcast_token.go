...
func (db *DB) CreateBroadcastToken(userID, name string, botIDs json.RawMessage) (*store.BroadcastToken, error) {
	...
	_, err := db.Exec(`INSERT INTO broadcast_tokens (id, user_id, name, token, bot_ids) VALUES ($1,$2,$3,$4,$5)`,
...
func (db *DB) GetBroadcastToken(id string) (*store.BroadcastToken, error) {
	return scanBroadcastToken(db.QueryRow(`SELECT id, user_id, name, token, bot_ids, created_at FROM broadcast_tokens WHERE id = $1`, id))
}

func (db *DB) GetBroadcastTokenByToken(token string) (*store.BroadcastToken, error) {
	return scanBroadcastToken(db.QueryRow(`SELECT id, user_id, name, token, bot_ids, created_at FROM broadcast_tokens WHERE token = $1`, token))
}

func (db *DB) ListBroadcastTokensByUser(userID string) ([]store.BroadcastToken, error) {
	rows, err := db.Query(`SELECT id, user_id, name, token, bot_ids, created_at FROM broadcast_tokens WHERE user_id = $1 ORDER BY created_at DESC`, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) UpdateBroadcastToken(id, name string, botIDs json.RawMessage) error {
	_, err := db.Exec(`UPDATE broadcast_tokens SET name=$1, bot_ids=$2 WHERE id=$3`, name, string(botIDs), id)
	return err
}

func (db *DB) DeleteBroadcastToken(id string) error {
	_, err := db.Exec(`DELETE FROM broadcast_tokens WHERE id = $1`, id)
	return err
}

func (db *DB) RegenerateBroadcastToken(id string) (string, error) {
	token := "bc_" + generateToken(16)
	_, err := db.Exec(`UPDATE broadcast_tokens SET token=$1 WHERE id=$2`, token, id)
	return token, err
}
